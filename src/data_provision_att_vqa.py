...
class DataProvisionAttVqa:
    def __init__(self, data_folder, feature_file):
        ...
        for split in self._splits:
            with open(os.path.join(data_folder,split)+'.pkl','rb')as f:
                split_question_id = pkl.load(f,encoding='iso-8859-1')
                split_image_id = pkl.load(f,encoding='iso-8859-1')
                split_question = pkl.load(f,encoding='iso-8859-1')
                split_answer = pkl.load(f,encoding='iso-8859-1')
                split_answer_counter = pkl.load(f,encoding='iso-8859-1')
                split_answer_label = pkl.load(f,encoding='iso-8859-1')
            idx = [i for i in range(split_question.shape[0])]
    ...
    def random_shuffle(self):
        for split in self._splits:
            if not (split is 'val2' or split is 'val2_all'):
                idx = [i for i in range(len(self._question[split]))]
                random.shuffle(idx)
    ...
    def iterate_batch_with_counter(self, partition, batch_size):
        ...
        while current + batch_size <= self._question[partition].shape[0]:
            ...
            batch_question = self._question[partition][current :
                                                       current + batch_size]
            # index - 1 as query for image fe   ature
            batch_image_feat = self._image_feat[batch_image_id]
            batch_image_feat = batch_image_feat.todense()
